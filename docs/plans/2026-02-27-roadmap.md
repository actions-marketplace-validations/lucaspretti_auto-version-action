...
| Item | Priority | Effort | Depends on | Owner | Status |
|------|----------|--------|------------|-------|--------|
| Refactor `bump-version.sh` to use `sed`/`jq` instead of `npm version` | P0 | 1w | Phase 1 | — | **Done** |
| Support `composer.json` (Drupal/PHP) | P0 | 3d | Refactor bump | — | **Done** |
| Support `pyproject.toml` (Python) | P1 | 3d | Refactor bump | — | **Done** |
| Support plain `VERSION` file (generic) | P2 | 1d | Refactor bump | — | **Done** |
| Add `version-file-type` input (auto-detect or explicit) | P1 | 2d | All format support | — | **Done** (auto-detect by filename, no new input needed) |
| Adopt in Drupal and mixed repos at EPO | P0 | 1w | composer.json support | TBD | Pending |
